...
- Os dias devem estar contidos em uma tag <li> , e todos devem ter a classe day . Ex: 
<li class="day">3</li>
...
function createDaysofTheMonth() {
  ...
  for (let index = 0; index < dezDaysList.length; index += 1) {
    ...
    listaDia.appendChild(dayListItem);
  }
}
...
function createHoliday(nameHoliday) {
  const divBotao = document.querySelector('.buttons-container');
  const listHoliday = document.createElement('button');
  const botaoID = 'btn-holiday';

  listHoliday.innerHTML = nameHoliday;
  listHoliday.id = botaoID;
  divBotao.appendChild(listHoliday);
}
createHoliday('Feriados');

// =======================================================================================
/* Exercício 3:
Implemente uma função que adicione ao botão "Feriados" um evento de "click" que muda a cor 
...
function mostraFeriado() {
  const containerB = document.getElementById('btn-holiday');
  const listHoliday = document.querySelectorAll('.holiday');
  const colorGray = 'rgb(238,238,238)';
  const colorGreen = 'green';

  containerB.addEventListener('click', function() {
    for(let index = 0; index < listHoliday.length; index += 1){
      if(listHoliday[index].style.backgroundColor === colorGreen) {
        listHoliday[index].style.backgroundColor = colorGray;
      } else {
        listHoliday[index].style.backgroundColor = colorGreen;
      }
    }
  })
}
...
- Adicione este botão como filho/filha da tag <div> com classe "buttons-container". */
function createFriday(nameFriday) {
  const divBotao = document.querySelector('.buttons-container');
  const buttonFriday = document.createElement('button');
  const botaoID = 'btn-friday';

  buttonFriday.innerHTML = nameFriday;
  buttonFriday.id = botaoID;
  divBotao.appendChild(buttonFriday);
}
...
function mostraTexto() {
  const containerB = document.getElementById('btn-friday');
  const listFriday = document.querySelectorAll('.friday');
  const modificaText = 'Acabou!';
  const dayFridays = [4, 11, 18, 25];

  containerB.addEventListener('click', function() {
    for(let index = 0; index < listFriday.length; index += 1){
      if(listFriday[index].innerText !== modificaText) {
        listFriday[index].innerText = modificaText;
      } else {
        listFriday[index].innerText = dayFridays[index];
      }
    }
  })
}
...
dia do mês no calendário, o texto desse dia deve aumentar e, quando o ponteiro do mouse 
...
// =======================================================================================
/* Exercício 7:
Implemente uma função que adiciona uma tarefa personalizada ao calendário. A função deve 
receber como parâmetro a string com o nome da tarefa (ex: "cozinhar") e criar 
...
"my-tasks" . */
function tarefa(nomeTarefa) {
  const myTasks = document.querySelector('.my-tasks');
  const listTarefa = document.createElement('span');
  listTarefa.innerHTML = nomeTarefa;
  
  myTasks.appendChild(listTarefa);
}
tarefa("cozinhar");

// =======================================================================================
/* Exercício 8:
Implemente uma função que adiciona uma legenda com cor para a tarefa criada no exercício 
anterior. Esta função deverá receber como parâmetro uma string ("cor") e criar 
...
classe "my-tasks". */
function corTask(nameCor) {
  const myTasks = document.querySelector('.my-tasks');
  const listTask = document.createElement('div');
  listTask.style.backgroundColor = nameCor;
  //listTask.classList.add('task');
  listTask.classList = 'task';
  myTasks.appendChild(listTask);
}
corTask('purple');
...
function addClass() {
  const recuperaTask = document.querySelector('.task');
  const selectedTask = document.getElementsByClassName('task selected');

  recuperaTask.addEventListener('click', function(event){
